...
type UserRepo[T any] interface {
	...
	UpdateByUid(uid string, entity *T, fields []string) error
...
// update
func (u *UserRepoImpl[T]) UpdateByUid(uid string, entity *T, fields []string) error {
	db := u.DB.Model(entity).Where("uuid =?", uid)
	if len(fields) > 0 {
		db = db.Select(fields)
	}
	if er := db.Updates(entity).Error; er != nil {
		log.Println("[UserRepoImpl] UpdateByUid error: ", er)
		return er
	}
	log.Printf("[UserRepoImpl] UpdateByUid success: %v", entity)
	return nil
}
